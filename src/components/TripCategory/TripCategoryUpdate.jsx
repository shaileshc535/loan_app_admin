...
import {
  loadSingleTripCategory,
  updateTripCategory,
} from "../../redux/action/tripCategoryAction";
import { ToastContainer, toast } from "react-toastify";
import "react-toastify/dist/ReactToastify.css";

function TripCategoryUpdate({ id, showChange }) {
  ...
  useEffect(() => {
    dispatch(loadSingleTripCategory(id));
  }, []);

  const tripCategory = useSelector((state) => state.tripCategory.singledata);

  useEffect(() => {
    if (tripCategory) {
      setState({ ...tripCategory });
    }
  }, [tripCategory]);
  ...
  const handleSubmit = (e) => {
    ...
    if (!type_name) {
      ...
      dispatch(updateTripCategory(id, state));
      showChange();
    }
  };
...
export default TripCategoryUpdate;
